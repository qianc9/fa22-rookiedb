...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        BPlusNode bPlusNode = BPlusNode.fromBytes(metadata, bufferManager, treeContext, children.get(idx));
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        BPlusNode bPlusNode = BPlusNode.fromBytes(metadata, bufferManager, treeContext, children.get(childIdx));
        Optional<Pair<DataBox, Long>> splitInfo = bPlusNode.put(key, rid);
        if (!splitInfo.isPresent()) {
            return splitInfo;
        }
        return insertInnerNode(splitInfo.get().getFirst(), splitInfo.get().getSecond());
    }

    private Optional<Pair<DataBox, Long>> insertInnerNode(DataBox key, Long rightSibling) {
        int insertIdx = numLessThan(key, keys);
        keys.add(insertIdx, key);
        children.add(insertIdx + 1, rightSibling);
        ...
        sync();
        return Optional.of(new Pair<>(splitKey, rightInnerNode.getPage().getPageNum()));
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        Optional<Pair<DataBox, Long>> childSplitInfo = rightMostChild.bulkLoad(data, fillFactor);
        if (!childSplitInfo.isPresent()) {
            // child node does not split
            return childSplitInfo;
        }
        Optional<Pair<DataBox, Long>> parentSplitInfo = insertInnerNode(childSplitInfo.get().getFirst(), childSplitInfo.get().getSecond());
        if (!parentSplitInfo.isPresent()) {
            // parent node (i.e. the current inner node) does not split
            return bulkLoad(data, fillFactor);
        }
        return parentSplitInfo;
    }
